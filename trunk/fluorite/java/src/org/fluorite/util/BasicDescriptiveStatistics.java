// $Id: BasicDescriptiveStatistics.java,v 1.3 2010/12/13 18:59:25 cmzmasek Exp $
//
...
public class BasicDescriptiveStatistics implements DescriptiveStatistics {
    ...
    public String asSummary() {
        if ( getN() > 1 ) {
            return arithmeticMean() + DescriptiveStatistics.PLUS_MINUS
                    + sampleStandardDeviation() + " [" + getMin() + "..."
                    + getMax() + "]";
        }
        else {
    ...
    public String getSummaryAsString() {
        ...
        return "" + mean + ( ( char ) 177 ) + sd + " [" + getMin() + "..."
                + getMax() + "]";
    }
    ...
    public double sampleStandardUnit( final double value ) {
        validate();
        return BasicDescriptiveStatistics.sampleStandardUnit( value,
                arithmeticMean(), sampleStandardDeviation() );
    }
    ...
    public double sampleVariance() {
        ...
        if ( getN() < 2 ) {
            throw new ArithmeticException(
                    "attempt to calculate sample variance for less then two values" );
        }
    ...
    public String toString() {
        ...
        sb.append( "n                       : " + getN() );
        if ( getN() > 1 ) {
            sb.append( FluoriteUtil.LINE_SEPARATOR );
    ...
    private void validate() throws ArithmeticException {
        if ( getN() < 1 ) {
            throw new ArithmeticException(
                    "attempt to get a result from empty data set statistics" );
        }
    }

    public static int[] performBinning( final double[] values,
            final double min, final double max, final int number_of_bins ) {
        if ( min >= max ) {
     ...
     */
    public static double sampleStandardUnit( final double value,
